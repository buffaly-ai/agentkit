...
public sealed class AgentConversation
{
    ...
    internal async ValueTask<IDisposable> EnterTurnAsync(CancellationToken ct) { await _turnLock.WaitAsync(ct); return new Releaser(_turnLock); }
    internal void Add(AgentMessage m) => _messages.Add(m);
